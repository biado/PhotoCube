  ...
  return (
    <div className="search results">
      ...
      >
        ...
        {props.options.map((o) => (
          <option key={o.NodeId} value={JSON.stringify(o)}>
            {o.NodeName}:{o.ParentnodeName}
          </option>
        ))}
      </select>
    </div>
 ...
 */
export const HierarchyExplorer = (props: {
    ...
    const options = response.map(
      (node: Node) =>
        ({
          ...
          NodeName: node.name,
          ParentnodeName:
            node.parentNode !== null ? node.parentNode!.name : null,
        } as Option)
    );
  ...
  async function onEnterPressed(e: React.KeyboardEvent) {
    if (e.charCode === 13) {
      ...
      const sorted = options.sort((a: Option, b: Option) =>
        a.NodeName > b.NodeName ? 1 : b.NodeName > a.NodeName ? -1 : 0
      );
      console.log(options);
  ...
  return (
    <div className="Filter">
      ...
      {options.length > 0 ? (
        <SearchResults options={options} onOptionSelected={onOptionSelected} />
      ) : null}
      {selectedNode !== null ? (
        <HierarchyBrowser
