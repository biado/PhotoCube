...
export default class CubeBrowser extends React.Component<{
  ...
  private onMouseMove = (event: MouseEvent) => {
    // the x/y/z boundries of the axis
    let axisFrom: number = 0;
    let axisTo: number = 10;

    if (this.pointerLockActive) {
      this.camera.position.y = 0.5;
      if (
        this.controls.moveForward &&
        axisFrom < this.camera.position.z &&
        this.camera.position.z < axisTo
      ) {
        this.camera.position.z += this.mouse.y / 4;
    ...
    if (intersects.length > 0 || sphereIntersects.length > 0) {
      ...
      let infoText: string = "";
      if (intersects.length > 0)
        infoText = "Number of photos: " + intersects[0].object.userData.size;
      else infoText = "Click to Drill Down";
      if (xDefined) {
        let intersectedObjectPosition;
        if (sphereIntersects.length > 0) {
          intersectedObjectPosition = sphereIntersects[0].object.position.x;
        } else {
          {
            intersectedObjectPosition = parseInt(
              intersects[0].object.userData.x
            );
          }
        }
        if (
          ...
          infoText +=
            ...
            (this.xAxis.TitleString +
              ": " +
              this.xAxis.Tags[intersectedObjectPosition - 1].name);
        } else if (this.xAxis.AxisType === AxisTypeEnum.Hierarchy) {
          infoText +=
            ",  X: " +
            (this.xAxis.TitleString +
              ": " +
              this.xAxis.Hierarchies[intersectedObjectPosition - 1].tag.name);
      ...
      if (yDefined) {
        let intersectedObjectPosition;
        if (sphereIntersects.length > 0) {
          intersectedObjectPosition = sphereIntersects[0].object.position.y;
        } else {
          {
            intersectedObjectPosition = parseInt(
              intersects[0].object.userData.y
            );
          }
        }
        if (
          ...
          infoText +=
            ...
            (this.yAxis.TitleString +
              ": " +
              this.yAxis.Hierarchies[intersectedObjectPosition - 1].tag.name);
      ...
      if (zDefined) {
        let intersectedObjectPosition;
        if (sphereIntersects.length > 0) {
          intersectedObjectPosition = sphereIntersects[0].object.position.z;
        } else {
          {
            intersectedObjectPosition = parseInt(
              intersects[0].object.userData.z
            );
          }
        }
        if (
          ...
          infoText +=
            ...
            (this.zAxis.TitleString +
              ": " +
              this.zAxis.Hierarchies[intersectedObjectPosition - 1].tag.name);
