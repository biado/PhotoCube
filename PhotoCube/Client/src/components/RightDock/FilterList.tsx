        ...
        <div className={props.className} id="filterlist-flexbox">
            ...
            <div id="filterlist-container">
                <ul className="filter list scrollable">
                    ...
                    {props.activeFilters.map(filter => (filter.type !== "day of week" && filter.type !== "time" ) ?
                    <li key={filter.id}>{filter.max? filter.name+"-"+filter.max:filter.name}<button className="clear button" onClick={() => props.onFilterRemoved(filter.id)}>
